...
- **Native PTY multiplexing.** The Rust daemon talks to PTYs directly (`openpty` on Unix, `ConPTY` on Windows). `@suzuke/agend` shells out to `tmux new-window` for every backend launch and inherits every tmux gotcha (server crashes, stale window IDs, attach quirks). On Rust, the daemon's own TUI is the multiplexer.
...
- [ ] **Cross-instance dispatch.** Send `delegate_task` from one agent to another. The recipient sees the `[AGEND-MSG]` system reminder and can drain via `inbox`. `describe_message(message_id=…)` confirms the recipient picked up.
